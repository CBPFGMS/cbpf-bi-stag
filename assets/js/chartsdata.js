...
const ipNames = ["ingo", "ngo", "o", "un"],
	...
	partnerLongNames = ["international ngo", "national ngo", "national partners", "red cross/red crescent society", "un agency", "others"],
	allocationTypes = ["standard", "reserve"],
	sectorLongNames = ["education", "emergency shelter and nfi", "health", "multi-sector", "protection", "water sanitation hygiene", "food security", "early recovery", "nutrition", "camp coordination / management", "logistics", "coordination and support services", "emergency telecommunications", "covid-19", "multi-purpose cash"],
...
const filesURLs = [{
	...
	dataFilters: [{
		...
		type: "string",
		filterFunction: str => allocationTypes.includes(str.toLowerCase()) //the allocation type must be one of the allocation types in the allocationTypes array
	}, {
	...
	dataFilters: [
		{
			...
			filterFunction: n => typeof n === "number" ? n > 0 && n <= maxFundId : ipNames.includes(n.toLocaleLowerCase()) //a valid ID in the funds master table or a valid IP name
		}, {
			name: "target",
			type: "string",
			filterFunction: str => subIpNames.includes(str.toLocaleLowerCase()) //a valid subIP name
		}, {
			name: "value",
			type: "number",
			filterFunction: n => n >= 0 //the dollar value must not be negative
	...
	dataFilters: [
		...
		},
		{
			...
			type: "string",
			filterFunction: str => partnerLongNames.includes(str.toLocaleLowerCase())
		},
		{
			name: "PooledFundName",
			type: "string",
	...
	dataFilters: [
		...
		},
		{
			name: "Cluster",
			type: "string",
			filterFunction: str => sectorLongNames.includes(str.toLocaleLowerCase()) //the sector must be one of the sectors in the sectorLongNames array
		},
		{
	...
	dataFilters: [
		{
			...
			type: "string",
			filterFunction: str => GenderMarkerGroups.includes(str.toLocaleLowerCase())
		}, {
			name: "GenderMarkerCode",
			type: t => typeof t === "number" || typeof t === "string", //the source can be either a number or a string
			filterFunction: n => typeof n === "number" ? n >= 0 && n <= 4 : GenderMarkerCodes.includes(n.toLocaleLowerCase()) //the code must be between 0 and 4 or one of the accepted strings
		}, {
			name: "TotalProjects",
			type: "number",
			filterFunction: n => n >= 0 //the number of projects must not be negative
...
filesURLs.forEach(file => {
	cbpfbiDataObject[file.name] = fetchFile(file.name, file.url, file.format, file.autoType, file.dataFilters);
});
...
function fetchFile(fileName, url, method, autoType, dataFilter) {
	if (localStorage.getItem(fileName) &&
		...
		const fetchedData = method === "csv" ? d3.csvParse(JSON.parse(localStorage.getItem(fileName)).data, autoType ? d3.autoType : null) :
			JSON.parse(localStorage.getItem(fileName)).data;
		console.log(`%cInfo: data file ${fileName} retrieved from local storage`, consoleStyle);
		return Promise.resolve(fetchedData);
	} else {
		const fetchMethod = method === "csv" ? d3.csv : d3.json;
		const rowFunctionWrapper = method === "csv" ? d => verifyRow(d, dataFilter, url, autoType) : null;
		return fetchMethod(url, rowFunctionWrapper).then(fetchedData => {
			try {
				...
				if (!isPfbiSite) console.warn(`Error saving the file ${fileName} in the local storage`);
			};
...
function verifyRow(obj, dataFilter, url, autoType) {
	...
	if (dataFilter) {
		...
		validRow = dataFilter.every(column => {
			...
			thisRule = column;
			if (column.type) {
				typeResult = typeof column.type === "function" ? column.type(copiedObj[column.name]) : typeof copiedObj[column.name] === column.type;
			} else {
				typeResult = true;
			};
			if (typeResult) filterResult = column.filterFunction ? column.filterFunction(copiedObj[column.name]) : true;
